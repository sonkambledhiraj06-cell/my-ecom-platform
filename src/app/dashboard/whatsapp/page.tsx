  ...
  const [chats] = useState([
    {
      ...
      lastMessage: "Is my order ORD-8921 shipped yet?",
      time: "14:32",
      unread: 1,
      orderRef: "ORD-8921",
    },
    {
      ...
      lastMessage: "Thank you for fast delivery! Loved the product.",
      time: "12:40",
      unread: 0,
      orderRef: "ORD-8922",
    },
    {
      ...
      lastMessage: "Can I pay via UPI on delivery?",
      time: "10:15",
      unread: 0,
      orderRef: "ORD-8923",
    },
          ...
          <div className="flex-1 divide-y divide-gray-100 overflow-y-auto">
            {chats.map((chat) => (
              <button
                ...
                className={`flex w-full cursor-pointer flex-col gap-1 p-4 text-left transition hover:bg-white ${activeChat.id === chat.id ? "border-l-4 border-emerald-500 bg-white shadow-sm" : ""}`}
              >
                ...
                <span className="line-clamp-1 text-xs text-gray-600">{chat.lastMessage}</span>
                <span className="mt-1 flex items-center justify-between">
                  <span className="rounded bg-gray-100 px-1.5 py-0.5 font-mono text-[10px] text-gray-600">{chat.orderRef}</span>
                  {chat.unread > 0 ? (
        ...
        <div className="flex h-full flex-col bg-white md:col-span-2">
          <div className="flex items-center justify-between border-b bg-gray-50/30 p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="text-sm font-semibold text-gray-900">{activeChat.name}</h4>
                <p className="text-xs text-gray-500">{activeChat.phone}</p>
              </div>
            </div>
            <button
              type="button"
          ...
          <div className="flex-1 space-y-4 overflow-y-auto bg-slate-50/50 p-4">
            <div className="my-2 flex flex-col items-center">
              <span className="rounded-full bg-gray-200 px-2 py-0.5 text-[10px] font-semibold uppercase text-gray-600">Today</span>
            </div>
            <div className="flex justify-start">
              <div className="max-w-xs space-y-1 rounded-2xl rounded-tl-none border bg-white p-3 text-xs text-gray-800 shadow-sm md:max-w-md">
                ...
                <span className="block text-right text-[10px] text-gray-400">{activeChat.time}</span>
              </div>
            </div>
            <div className="flex justify-end">
              <div className="max-w-xs space-y-1 rounded-2xl rounded-tr-none bg-emerald-600 p-3 text-xs text-white shadow-sm md:max-w-md">
                ...
                <p>Hello {activeChat.name}, your order {activeChat.orderRef} is currently being packed and will be dispatched today via BlueDart.</p>
                <span className="flex items-center justify-end gap-1 text-[10px] opacity-75">14:35 <CheckCheck size={12} /></span>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-2 border-t bg-white p-3">
            ...
            <div className="flex items-center gap-2">
              <input type="text" aria-label="Custom message" placeholder="Type custom message or reply..." className="flex-1 rounded-lg border px-4 py-2.5 text-xs outline-none focus:border-emerald-500" />
